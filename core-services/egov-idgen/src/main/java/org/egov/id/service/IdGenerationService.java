...
public class IdGenerationService {
	...
	// by default 'idformat' will be taken from MDMS. Change value of 'ismdms.on' to
	// 'false'
	...
	private List generateIdFromIdRequest(IdRequest idRequest, RequestInfo requestInfo) throws Exception {
		...
		if (idRequest.getFormat().equals(IMC_PROPERTYID_FORMAT) || idRequest.getFormat().equals(IMC_ACKID_FORMAT)
				|| idRequest.getFormat().equals(IMC_RECIEPTID_FORMAT)) {
	...
	/**
	 * Description : This method to generate Id when format is unknown and select
	 * MDMS or DB.
	 *
	 * @param idRequest
	 * @param requestInfo
	...
	private String getIdFormatfromDB(IdRequest idRequest, RequestInfo requestInfo) throws Exception {
		...
		try {
			...
			String rs = jdbcTemplate.queryForObject(idSelectQuery.toString(), new Object[] { idName, tenantId },
					String.class);
	...
	private List getFormattedId(IdRequest idRequest, RequestInfo requestInfo, boolean autoCreateNewSeqFlag)
		...
		for (int i = 0; i < count; i++) {
			...
			for (String attributeName : matchList) {
				...
				if (attributeName.length() >= 3 && attributeName.substring(0, 3).equalsIgnoreCase("seq")) {
				...
				} else if (attributeName.length() >= 2 && attributeName.substring(0, 2).equalsIgnoreCase("fy")) {
					idFormat = idFormat.replace("[" + attributeName + "]",
							generateFinancialYearDateFormat(attributeName, requestInfo));
				} else if (attributeName.length() >= 2 && attributeName.substring(0, 2).equalsIgnoreCase("cy")) {
					idFormat = idFormat.replace("[" + attributeName + "]",
							generateCurrentYearDateFormat(attributeName, requestInfo));
				} else if (attributeName.length() >= 4 && attributeName.substring(0, 4).equalsIgnoreCase("city")) {
					if (cityName == null) {
						cityName = mdmsService.getCity(requestInfo, idRequest);
					}
					idFormat = idFormat.replace("[" + attributeName + "]", cityName);
				} else if (attributeName.length() >= 2 && attributeName.substring(0, 2).equalsIgnoreCase("YY")) {
					idFormat = idFormat.replace("[" + attributeName + "]", generateYY());
				} else if (attributeName.length() >= 2 && attributeName.substring(0, 2).equalsIgnoreCase("MM")) {
					idFormat = idFormat.replace("[" + attributeName + "]", generateMM());
				} else if (attributeName.length() >= 2 && attributeName.substring(0, 2).equalsIgnoreCase("DD")) {
					idFormat = idFormat.replace("[" + attributeName + "]", generateDD());
				} else {
					idFormat = idFormat.replace("[" + attributeName + "]",
	...
	/**
	 * Description : This method to generate current financial year in given format
	 *
	 * @param requestInfo
	...
	private String generateFinancialYearDateFormat(String financialYearFormat, RequestInfo requestInfo) {
		try {
			...
			throw new CustomException("INVALID_FORMAT",
					"Error while generating financial year in provided format. Given format invalid.");
			// throw new InvalidIDFormatException(propertiesManager.getInvalidIdFormat(),
			// requestInfo);
	...
	private String generateCurrentYearDateFormat(String dateFormat, RequestInfo requestInfo) {
		try {
			...
			throw new CustomException("INVALID_FORMAT",
					"Error while generating current year in provided format. Given format invalid.");
			// throw new InvalidIDFormatException(propertiesManager.getInvalidIdFormat(),
			// requestInfo);
	...
	private List<String> generateSequenceNumber(String sequenceName, RequestInfo requestInfo, IdRequest idRequest,
			boolean autoCreateNewSeqFlag) throws Exception {
		Integer count = getCount(idRequest);
